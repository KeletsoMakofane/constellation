...
nodes_projects      <- "using periodic commit 1000 LOAD CSV FROM 'file:///project_nodes.csv' as row CREATE (m:Project {id : row[0], title : row[1], year : toInteger(row[2]), indirect_cost : toInteger(row[3]), direct_cost : toInteger(row[4]), total_cost : toInteger(row[5]), total_cost_subproject : toInteger(row[6])}) ;"
